...
function english2kana($englishName){
	...
	foreach($englishArray as $englishName){
		...
		if(array_key_exists($englishName, $nameEnArray)) $kanaName .= $nameEnArray[$englishName]." "; //リスト変換
		else {
...
function kana2kanji($kanaName, $kanjiCandidateMax){
	global $kanaArray;
	$kanaName = mb_substr($kanaName, 0, 12, ENCODE_TYPE); //13文字目以降は削除

	$kanjiCandidateArray2D = getKanjiCandidate($kanaName, $kanjiCandidateMax); //漢字候補のリストを取得
	$kanjiName = "";
...
function getKanjiCandidate($kanaName, $kanjiCandidateMax){
	global $kanaArray;
	if(empty($kanaName)) return array();
	...
	for($i=mb_strlen($kanaName, ENCODE_TYPE); $i>0; $i--){
		...
		if(array_key_exists($kanaNameSub1, $kanaArray)) return array_merge(array(array_slice($kanaArray[$kanaNameSub1], 0, $kanjiCandidateMax)), getKanjiCandidate($kanaNameSub2, $kanjiCandidateMax)); //前方をDB検索、後方は再帰
	}
	...
	return getKanjiCandidate($kanaName, $kanjiCandidateMax);
}

///// replaceKana: カナの分解に失敗した場合の修正 /////
function replaceKana($kanaLetter){
	if($kanaLetter==="ヂ") return "ジ";
	if($kanaLetter==="ヅ") return "ズ"; 
	if($kanaLetter==="ー") return null;
	if($kanaLetter==="ッ") return null;
	if($kanaLetter==="ャ") return "ヤ";
	if($kanaLetter==="ュ") return "ユ";
	if($kanaLetter==="ョ") return "ヨ";
	if($kanaLetter==="ン") return "ム";
	if($kanaLetter==="ァ") return "ア";
	if($kanaLetter==="ィ") return "イ";
	if($kanaLetter==="ゥ") return "ウ";
	if($kanaLetter==="ェ") return "エ";
	if($kanaLetter==="ォ") return "オ";
...
function getKanjiInfo($kanjiCandidateArray2D, $language){
	global $kanjiArray;
	if(empty($language)) $language = "en"; //デフォルトを英語に設定
	
	$kanjiInfoArray = array();
	foreach($kanjiCandidateArray2D as $kanjiCandidateArray) foreach($kanjiCandidateArray as $kanjiLetter){
...
function getGogyoInfo($katakanaName, $kanjiName, $gender, $language){
	...
	if(empty($gender)) $gender = "m"; //デフォルトを男性に設定
